...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  const hasPremiumPlano =
    (await user).publicMetadata.subscriptionPlan == "premium";
  if (!hasPremiumPlano) {
    throw new Error("You need a premium plan to generate AI reports");
  }

  // Caso não possua a chave da OpenAPI retorna um relatório local.
  ...
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar
   minha vida financeira. As transações estão divididas por ponto e vírgula. A estrutura de cada uma é
