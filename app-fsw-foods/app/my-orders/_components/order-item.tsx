...
const OrderItem = ({ order }: OrderItemProps) => {
   ...
   const handleRedoOrderClick = () => {
      for (const orderProduct of order.products) {
         addProductToCart({
            product: { ...orderProduct.product, restaurant: order.restaurant, quantity: orderProduct.quantity },
         })
      }
      ...
      <Card>
         <CardContent className="p-5">
            ...
            <div className="flex justify-between items-center pt-3">
               ...
               <Button variant="link" size="icon" className="h-5 w-5 text-black" asChild>
            ...
            <div className="space-y-2">
               {order.products.map(product => (
                  <div key={product.id} className="flex items-center gap-2">
                     ...
                     <span className="text-muted-foreground text-sm text-xs">{product.product.name}</span>
                  </div>
               ))}
            </div>
